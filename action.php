...
class action_plugin_tplmod extends DokuWiki_Action_Plugin {
    ...
    function register(Doku_Event_Handler $controller) {
        ...
        $controller->register_hook('TEMPLATE_SITETOOLS_DISPLAY', 'BEFORE', $this, 'action_link', ['site']);
    ...
    function __construct() {
        ...
        if (!file_exists($this->ui_priority_metafn)) {
            io_saveFile($this->ui_priority_metafn, serialize([]));
        }
    ...
    function handle_profile_form(Doku_Event $event, $param) {
        ...
        $_form .=
            ...
            "onclick='tplmod_setui_lang(this.form.tplmod_selector.value,this.form.tplmod_client.value,this.form.tplmod_selector);' />&nbsp;";
    ...
    function dwstarted(DOKU_EVENT $event, $param) {
        ...
        $acl_levels = ['NONE' => 0, 'READ' => 1, 'EDIT' => 2, 'CREATE' => 4, 'UPLOAD' => 8, 'DELETE' => 16];
        ...
        if (!empty($background_color)) {
            if ($background_color == 'default') {
                $background_color = $this->html_bg_color;
            }
        ...
        for ($i = 0; $i < count($ips); $i++) {
            ...
            if ($remote_addr == $addr) {
                if (!empty($names[$i])) {
                    ...
                    $JSINFO['tmplft_title'] = trim($names[0]);
                }
            }
        }
    }
    ...
    function tools() {
        ...
        if (isset($pagetools_conf)) {
            ...
            $JSINFO['tmplft_actions'] = implode(',', $mobile_ar);
        }
    ...
    function action_link(&$event, $param) {
        ...
        $event->data['items']['tplmod'] =
            '<li><a href="javascript:tplmod_toggle_aside();void(0);"  class="tplmodtoggle" rel="nofollow"   title="' .
            $name .
            '">' .
            $name .
            '</a></li>';
